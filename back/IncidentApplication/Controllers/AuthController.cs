...
namespace IncidentApplication.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterRequest request)
        {
            ...
            if (user == null)
            {
                return BadRequest(new ApiResponse<string>(1, "Account failed to be registered"));
            }
            else
            {
                Task task = await GenerateConfirmationEmailAsync(user);
                await task;
                return Ok(new ApiResponse<string>("Account registered successfully"));
            }
        }
        ...
        public async Task<IActionResult> Authenticate([FromBody] AuthenticateRequest request)
        {
            ...
            if (user == null)
            {
                return Unauthorized(new ApiResponse<string>(1, "Account failed to be registered"));
            } else
            {
                ...
                return Ok(new ApiResponse<AuthenticateResponse>(
                            new Payload<AuthenticateResponse>(
                                new AuthenticateResponse(accessToken, refreshToken, user.Id)), 
        ...
        public async Task<IActionResult> ConfirmEmailAsync([FromRoute] string id, [FromRoute] string code)
        {
            ...
            if (!emailConfirmed)
            {
                ...
                {
                    return Ok(new ApiResponse<string>("Email confirmed"));
                } else
                {
                    return BadRequest(new ApiResponse<string>(1, "Token is invalid or has expired"));
                }
            } else
            {
                return Ok(new ApiResponse<string>("Email already confirmed"));
            }
        ...
        public async Task<IActionResult> RefreshAccessToken([FromBody] RefreshAccessTokenRequest request)
        {
            ...
            {
                return NotFound(new ApiResponse<string>(5 ,"Token not found"));
            } else
            {
                if (refreshToken.UserId != request.Id)
                {
                    return BadRequest(new ApiResponse<string>(4, "User not matching"));
                } else
                {
                    if (refreshToken.ExpiryDate <= DateTime.Now)
                    {
                        return Unauthorized(new ApiResponse<string>(2, "Refresh token expired"));
                    } else
                    {
                        ...
                        if (user == null)
                        {
                            return BadRequest(new ApiResponse<string>(5, "Could not find user"));
                        }
                        else if (refreshToken.UsedCount != 0)
                        {
                            ...
                            return Unauthorized(new ApiResponse<string>(3, "Refresh token might be compromised"));
                        }
                        else
                        {
                            refreshToken.UsedCount += 1;
                            ...
                            return Ok(new ApiResponse<AuthenticateResponse>(
                                        new Payload<AuthenticateResponse>(
                                            new AuthenticateResponse(newAccessToken, newRefreshToken, user.Id)),
        ...
        public async Task<IActionResult> GetProfile([FromRoute] string id)
        {
            ...
            if (id != HttpContext.User.FindFirstValue(JwtClaimTypes.Id))
            {
                return Forbid();
            }
            ...
            if (user == null)
            {
                return NotFound(new ApiResponse<string>(5, "Failed to find user"));
            }
            else
            {
        ...
        public async Task<IActionResult> SendResetPasswordLinkAsync([FromBody] string email)
        {
            ...
            if (user == null)
            {
                return NotFound(new ApiResponse<string>(1, "Account not found"));
            }else
            {
                Task task = GenerateResetPasswordEmailAsync(user);
                await task;
                return Ok(new ApiResponse<string>("Reset password email is sent successfully"));
            }
        ...
        public async Task<IActionResult> ResetPassword([FromBody] ResetPasswordRequest request)
        {
            ...
            else
            {
                ...
                {
                    return Ok(new ApiResponse<string>("Password has been changed successfully"));
                } else
                {
                    return BadRequest(new ApiResponse<string>(1, "Token is invalid or has expired"));
                }
        ...
        private async Task<Task> GenerateConfirmationEmailAsync(ApplicationUser user)
        {
            ...
            string EmailConfirmationUrl = "https://localhost:5001/confirm-email?id=" + user.Id + "&code=" + code;
